...
def load_environment(environment_path: Path) -> BBTEnvironment:
    payload = json.loads(environment_path.read_text(encoding="utf-8"))
    return BBTEnvironment(
        image_path=str(payload["image_path"]),
        image_width=int(payload["image_width"]),
        image_height=int(payload["image_height"]),
        box_polygon=[tuple(point) for point in payload["box_polygon"]],
        ...
        testing_hand=str(payload["testing_hand"]),
...
def build_crossing_zone_polygon(
    ...
    extended_line = np.array([[top_x, top_y], [bottom_x, bottom_y]], dtype=np.float32)
...
def polygon_to_mask(image_shape: tuple[int, int] | tuple[int, int, int], polygon: list[Point]) -> np.ndarray:
...
def render_environment(image: np.ndarray, environment: BBTEnvironment, alpha: float = 0.28) -> np.ndarray:
    ...
    draw_text_block(preview, ["Start side"], origin=(max(environment.start_polygon[0][0] + 10, 15), max(environment.start_polygon[0][1] + 25, 30)), color=(70, 180, 70), font_scale=0.8)
    draw_text_block(preview, ["Target side"], origin=(max(environment.target_polygon[1][0] + 10, 15), max(environment.target_polygon[1][1] + 25, 30)), color=(40, 170, 240), font_scale=0.8)
    draw_text_block(preview, ["Crossing zone"], origin=(max(environment.crossing_zone_polygon[0][0] + 10, 15), max(environment.crossing_zone_polygon[0][1] + 25, 30)), color=(255, 120, 20), font_scale=0.8)
    return preview
...
def save_region_masks(image: np.ndarray, environment: BBTEnvironment, output_dir: Path) -> dict[str, Path]:
...
def annotate_environment(
    ...
    start_polygon, target_polygon = build_side_polygons(box_polygon, partition_line, resolved_start_side)
...
class _EnvironmentSelector:
    ...
    def _on_mouse(self, event: int, x: int, y: int, _flags: int, _param: object) -> None:
        if event != cv2.EVENT_LBUTTONDOWN:
            return
        point = (int(round(x / self.display_scale)), int(round(y / self.display_scale)))
        if self.step == "box" and len(self.box_points) < 4:
            self.box_points.append(point)
        elif self.step == "partition" and len(self.partition_points) < 2:
            self.partition_points.append(point)
    ...
    def _scale_points(self, points: list[Point]) -> list[Point]:
        return [(int(round(x * self.display_scale)), int(round(y * self.display_scale))) for x, y in points]
